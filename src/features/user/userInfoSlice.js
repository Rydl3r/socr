...
export const userInfoSlice = createSlice({
    ...
    reducers: {
        ...
        deleteFriend: (state, action) => {
            state.value.friends = state.value.friends.filter((friend) => friend !== action.payload)
        },
        addSentRequest: (state, action) => {
            state.value.sentRequests.push(action.payload)
        },
        deleteSentRequest: (state, action) => {
            state.value.sentRequests = state.value.sentRequests.filter((request) => request !== action.payload)
        },
        deleteRequest: (state, action) => {
            state.value.requests = state.value.requests.filter((request) => request !== action.payload)
        },
    },
})
