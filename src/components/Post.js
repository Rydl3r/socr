...
const Post = (props) => {
    ...
    const likePost = async () => {
        if (currentIsLogged === true) {
            if (currentLikes.includes(currentUserInfo.id)) {
                ...
                dispatch(removeLike({ likeAuthor: currentUserInfo.id, postId: props.data.id }))
    ...
    const deleteComment = async (commentToDelete) => {
        ...
        let newArr = commentsData.filter((comment) => JSON.stringify(comment) !== JSON.stringify(commentToDelete))
        setCommentsData(newArr)
        ...
        <Card sx={{ p: 3, m: 3 }}>
            ...
            <Box sx={{ display: "flex" }}>
                ...
                {props.data.author === currentUserInfo.id
                    ? <Button variant="outlined" color="error" sx={{ display: "flex", ml: "auto", my: 2 }} onClick={() => props.deletePost(props.data.id)}><CancelIcon sx={{ pr: 1 }}></CancelIcon>Delete Post</Button>
                    : ""
                }
            </Box>
            ...
            <CardContent>
                ...
                <Box sx={{ display: "flex", alignItems: "center", pb: 2 }}>
                    <Button color="info" variant="outlined" sx={{ display: "flex" }} onClick={() => likePost()}>{currentLikes.includes(currentUserInfo.id) ? <ThumbUpIcon></ThumbUpIcon> : <ThumbUpOutlinedIcon></ThumbUpOutlinedIcon>} </Button>
                    <Typography variant="h6" sx={{ mx: 2 }}>{currentLikes.length} {currentLikes.length === 1 ? "person" : "people"} liked this</Typography>
                </Box>
                <Box>
                    ...
                    {commentsData.length > commentsCount
                        ? <Button variant="contained" onClick={() => setCommentsCount(commentsCount + 3)}>Load More Comments</Button>
                        : <Box>
                            {commentsCount !== 3
                                ? <Button variant="contained" sx={{ display: "flex" }} onClick={() => setCommentsCount(3)}>Load Less</Button>
                                : ""}
                        </Box>}

                    {currentIsLogged
                        ? <Box sx={{ display: "flex", alignItems: "center" }}>
                            <TextField multiline={true} value={addingComment} onChange={(e) => setAddingComment(e.target.value)} id="outlined-basic" label="Comment" type="text" variant="outlined" sx={{ my: 1, mx: 1 }} />
                            <Button variant="contained" sx={{ display: "flex" }} onClick={() => addComment()}><AddIcon sx={{ pr: 1 }}></AddIcon>Add Comment</Button>
                        </Box>
                        : ""
                    }
